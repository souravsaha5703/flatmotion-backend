...
from controllers.supabase_controller import get_current_user_id,insert_chat,get_previous_prompts,update_chat
...
@router.put("/new_video_generate")
async def generate_new_video(req:ModificationRequest,user_id:str = Depends(get_current_user_id)):
    if validate_prompt(req.prompt):
        try:
            completion = client.chat.completions.create(
                model=os.getenv("MODEL"),
